...
function ret:Library(Name)
	...
	local XathenaGradient = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(76, 28, 138)), ColorSequenceKeypoint.new(1, Color3.fromRGB(131, 45, 161))}
...
function ui:Note(Title, Message, Error)
    ...
    -- Start off-screen LEFT by using a negative offset but relative scale 0 for X
    -- Using negative offset to put it just outside left screen edge, but never less than 0 scale
    Note.Position = UDim2.new(0, -Note.AbsoluteSize.X, 0, 0)
    Note.Frame.Position = UDim2.new(0, 0, 0, 0)  -- Frame inside Note, aligned top-left
    ...
    for i, v in pairs(Notifications:GetChildren()) do
        -- Move existing notifications up 42 pixels (relative to parent container)
        -- Keep X position the same (scale 0, offset whatever it was)
        local currentPos = rtbl[v]
        rtbl[v] = UDim2.new(currentPos.X.Scale, currentPos.X.Offset, currentPos.Y.Scale, currentPos.Y.Offset - 42)
        v:TweenPosition(rtbl[v], "Out", "Quad", 0.35, true)
    end

    -- Tween this note *in* from left to an onscreen position with 10px padding
    local targetPosition = UDim2.new(0, 10, 0, 0)
    Note:TweenPosition(targetPosition, "Out", "Quad", 0.35, true)
	...
	function ui:Window(rbrbrb)
		...
		function self:Toggle(name,b,f)
			local onc = XathenaGradient --ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(46, 59, 145)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(39, 49, 126))}
		...
		function self:Slider(n,min,max,default,precise,f)
			...
			OnToggleGradient.Color = XathenaGradient
		...
		function self:ToggleDropdown(n,de,fu)
			...
			function self2:Slider(n,min,max,default,precise,f)
				...
				OnToggleGradient.Color = XathenaGradient --ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(46, 59, 145)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(39, 49, 126))}
				OnToggleGradient.Rotation = 90
				OnToggleGradient.Name = "OnToggleGradient"
				OnToggleGradient.Parent = Slider_2
